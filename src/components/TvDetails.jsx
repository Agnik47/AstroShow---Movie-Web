...
import P_DeatilsLoader from "../Loader/P_DeatilsLoader";
...
const TvDetails = () => {
  ...
  return info && info.detail ? (
    <div
      className="relative w-full min-h-[270vh] lg:min-h-[230vh] text-white flex flex-col items-center bg-black "
      style={{
        ...
        <div className="flex flex-col gap-3 w-full md:w-2/3 text-center md:text-left">
          <h1 className="text-3xl font-bold">
            ...
            <span className="text-xl font-light text-gray-300 ml-2">
              ...
              {info.detail.status === "Ended"
                ? new Date(info.detail.last_air_date).getFullYear()
                : "Present"}
              )
            </span>
          </h1>
          ...
          <p className="text-md text-gray-300">
            Created by:{" "}
            {info.detail.created_by.map((creator) => creator.name).join(", ")}
          </p>
      ...
      <div className="w-full max-w-5xl px-5 md:px-10 mt-8">
        ...
        <div className="grid grid-cols-2 md:grid-cols-3 gap-4 bg-black bg-opacity-50 p-5 rounded-md shadow-md">
          <p>
            ...
            {info.detail.networks.map((network) => network.name).join(", ")}
          </p>
          <p>
            <strong>Languages:</strong> {info.detail.languages.join(", ")}
          </p>
          <p>
            <strong>Episodes:</strong> {info.detail.number_of_episodes}
          </p>
          <p>
            <strong>Seasons:</strong> {info.detail.number_of_seasons}
          </p>
          <p>
